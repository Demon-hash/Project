...
export const Products: FC<Properties> = ({ products }) => {
    ...
    const cached = useMemo(() => {
        return products?.map(
            ({ id, title, description, price, imageUrl, stock, color }) => (
    ...
    // <div className="grid grid-cols-3 gap-2 py-4">{cached}</div>

    return <div className="grid grid-cols-4 gap-2 py-4">{cached}</div>;
};
